...
While this discussion (of how to deal with potentially endogenous variables) is common for all regression analysis, it demonstrates here how error correlation can occur and why it might be important to account for it. Both model specifications (\code{fit_red} and \code{fit_commute}) produce almost identical insights in the post-estimation that follows and we proceed with the latter model.
...
We first define some helper functions to compute average treatment effects
%
<<utils>>=
...
These treatment effects can also be visualized in a pairwise comparison and at the individual level
%
<<hidden, echo=FALSE>>=
plot_treatment <- function(object, x, y, type, xlim = NULL, ylim = NULL, main = NULL,
  ...
  base_plot <- function() {
    ...
    for (i in seq_len(object$nReg)) {
      ...
      text(x = mpx[[i]], y = ylim[1], labels = round(mpx[[i]], digits),
           col = col[i], adj = c(-0.1, 1), cex = cex.text)
      abline(h = mpy[[i]], col = col[i], lty = lty, lwd = lwd)
      text(x = xlim[1], y = mpy[[i]], labels = round(mpy[[i]], digits),
...
\begin{figure}[t!]
\centering
<<plot-treatment, echo=FALSE, fig=TRUE, height=3, width=9>>=
fit <- fit_commute
...
g1 <- function() {
  plot_treatment(fit, x = 1, y = 2, type = "unlog-response",
...
g2 <- function() {
  plot_treatment(fit, x = 1, y = 3, type = "unlog-response",
...
g3 <- function() {
  plot_treatment(fit, x = 2, y = 3, type = "unlog-response",
...
gridExtra::grid.arrange(g1grob, g2grob, g3grob, nrow = 1)
@
