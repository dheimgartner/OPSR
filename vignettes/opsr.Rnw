...
\pkg{OPSR}'s architecture can be conceptualized as follows:
\begin{itemize}
...
\item After parsing the formula object and generating the model matrices, the Heckman two-step estimator is called in \fct{opsr\_2step} to generate reasonable starting values.
