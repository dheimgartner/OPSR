...
\begin{leftbar}
daniehei: See overleaf tutorial \url{https://www.overleaf.com/learn/latex/Aligning_equations_with_amsmath}.
\end{leftbar}
...
We first illustrate how to specify a model using the \pkg{Formula}'s extended model formulas and simulated data. Then the main functionality of the package is demonstrated. We conclude this section by demonstrating some nuances, reproducing the core model of \cite{Wang+Mokhtarian:2024}.
...
Models can be compared side-by-side using the \pkg{texreg} package \citep{Leifeld:2013}, which also allows the user to build production ready tables as illustrated later.
%
<<texreg>>==
...
Finally, the key interest of an OPSR study almost certainly is the estimation of treatment effects which relies on (counterfactual) conditional expectations as already noted in the mathematical description.
%
<<predict>>==
...
@
%
where \code{p1} is the result of applying Equation~\ref{eq:cond-exp} and \code{p2} results from Equation~\ref{eq:counterfact-exp}. The following \code{type} arguments are available
\begin{itemize}
...
With help of the \pkg{texreg} package, production ready tables (in various output formats) can be built with ease.
%
<<replica, results=tex>>=
texreg::texreg(
...
\begin{leftbar}
daniehei: TU+ case study. Intended to demonstrate how to use \pkg{OPSR} in a real-(or research-) world example. Don't comment on what a function does but interpret the output substantively.
\end{leftbar}
