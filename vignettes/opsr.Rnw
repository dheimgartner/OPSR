...
make_row <- function(run_id) {
  ...
  paste(c(run_labels[run_id], vals), collapse = " & ")
}

data_rows <- paste0(sapply(levels(fiml$run), make_row), " \\\\")

writeLines(c(
  "\\begin{table}[htbp]",
  "\\centering",
  paste0("\\caption{\\label{tab:sim} FIML mean $\\hat{\\beta}$ and 95\\% CI ",
         "coverage at $N = 1000$ across three regimes. True $\\beta = 1$.}"),
  "\\begin{tabular}{lccc}",
  "\\toprule",
  "Condition & Regime~1 & Regime~2 & Regime~3 \\\\",
  "\\midrule",
  data_rows,
  "\\bottomrule",
  "\\end{tabular}",
...
data_rows2 <- vapply(seq_len(nrow(diag_r2)), function(i) {
  run_id <- as.character(diag_r2$run[i])
  sprintf("%s & %.0f & %.1f & %s \\\\",
          run_labels2[run_id],
          diag_r2$cond_number[i],
          diag_r2$profile_range[i],
          fmt_p(diag_r2$lr_pval[i]))
}, character(1))

writeLines(c(
  "\\begin{table}[htbp]",
  "\\centering",
  paste0("\\caption{\\label{tab:sim-diag} Diagnostics for one representative ",
         "dataset per condition ($N = 1000$). Profile range is the ",
         "log-likelihood drop across the $\\rho$ grid for the interior ",
         "regime~2; smaller values indicate a flatter (weaker) profile.}"),
  "\\begin{tabular}{lccc}",
  "\\toprule",
  "Condition & Cond.~number & Profile range & LR $p$-value \\\\",
  "\\midrule",
  data_rows2,
  "\\bottomrule",
  "\\end{tabular}",
