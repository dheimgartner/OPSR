...
Throughout the text we use the convention that OPSR refers to the general methodology, while \pkg{OPSR} refers specifically to the package.
...
\item The coefficients table reports robust standard errors based on the sandwich covariance matrix as computed with help of the \pkg{sandwich} package \citep{}. \code{rob = FALSE} reports conventional standard errors.
...
Models can be compared side-by-side using the \pkg{texreg} package \citep{}, which also allows the user to build production ready tables as illustrated later.
%
<<texreg>>==
