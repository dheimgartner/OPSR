...
%\VignetteIndexEntry{OPSR: A Package for Estimating Ordered Probit Switching Regression Models in R}
%\VignetteDepends{OPSR,MASS,texreg,gridExtra,gridGraphics,scales,mvtnorm}
%\VignetteKeywords{ordered probit switching regression, endogenous switching regression, Heckman selection, selection bias, treatment effect, R}
...
The identification of $\rho_j$ is a critical practical concern. From Equation~\ref{eq:cond-exp} we see that if the inverse Mills ratio term is collinear with $\boldsymbol{X_j}$, the outcome regression cannot separately identify $\boldsymbol{\beta_j}$ and $\rho_j\sigma_j$. This collinearity arises whenever $\boldsymbol{W}$ and all $\boldsymbol{X_j}$ share exactly the same variables: the correction term becomes a near-linear function of $\boldsymbol{X_j\beta_j}$, so $\rho_j$ is identified only through the curvature of $\Phi(\cdot)$ --- a distributional assumption rather than genuine variation in the data \citep{Chiburis+Lokshin:2007}. This motivates the standard requirement that at least one variable in $\boldsymbol{W}$ must be excluded from all $\boldsymbol{X_j}$: the \emph{exclusion restriction} or \emph{instrument}.

The severity of functional-form-only identification depends on the regime structure. \citet{Chiburis+Lokshin:2007} show that for \emph{interior} regimes (two finite thresholds $\kappa_{j-1}$ and $\kappa_j$), the generalized inverse Mills ratio is nearly linear in $\Wg$, rendering functional-form identification practically useless at empirically relevant sample sizes. For \emph{exterior} regimes (one infinite threshold), the nonlinearity is stronger, but still fragile. Consequence: a weak or absent exclusion restriction leads to inflated standard errors for $\rho_j$, biased estimates of $\boldsymbol{\beta_j}$, and unreliable treatment effects --- even if the optimizer converges without numerical warning.

Three failure modes arise in practice, with different consequences:
\begin{enumerate}
\item \textbf{No exclusion restriction}: all variables in $\boldsymbol{W}$ also appear in all $\boldsymbol{X_j}$. Identification relies entirely on functional form. FIML estimates appear precise but are downward biased, with coverage collapsing well below 95\%; the two-step estimator has enormous variance.
\item \textbf{Weak instrument}: the excluded variable has negligible first-stage predictive power for $Z$. Numerically similar to the no-exclusion case; the condition number of the Hessian inflates and the profile likelihood for $\rho_j$ is near-flat.
\item \textbf{Invalid exclusion restriction}: the excluded variable directly affects $y_j$ and should appear in $\boldsymbol{X_j}$. Both FIML and the two-step estimator converge confidently to the wrong answer (bias can exceed 80\%; coverage collapses to 0\%). No model-internal diagnostic can detect this failure mode --- it requires substantive subject-matter reasoning.
\end{enumerate}

\pkg{OPSR} provides two complementary tools for failure modes~(1) and~(2): a likelihood-ratio test for instrument relevance (\fct{opsr\_lr\_instrument}) and a per-regime profile log-likelihood for $\rho_j$ (\fct{opsr\_profile\_rho}), combined in \fct{opsr\_test\_exclusion\_restriction}. These are demonstrated in Section~\ref{sec:diagnostics} and Section~\ref{sec:cs-identification}; the simulation evidence for all three failure modes is presented in Section~\ref{sec:simulation}.
...
\item These are then passed together with the data to the basic computation engine \fct{opsr.fit}. The main estimates are retrieved using maximum likelihood estimation by passing the log-likelihood function \fct{loglik\_cpp} (Equation~\ref{eq:log-lik}) to \fct{maxLik} from the \pkg{maxLik} package \citep{Henningsen+Toomet:2011}. No analytical gradient or Hessian is implemented; \fct{maxLik} uses numerical finite-difference approximations by default. Reported standard errors and Wald statistics are derived from the negative inverse of this numerical Hessian. Near identification failure the Hessian may be numerically imprecise, which is flagged by the condition number diagnostic in \fct{opsr\_diagnostics}.
...
We first illustrate how to specify a model using \pkg{Formula}'s extended syntax and simulated data and demonstrate the main functionality of the package (Section~\ref{sec:opsr-core}). We then demonstrate the convergence diagnostics and identification tests (Section~\ref{sec:diagnostics}), followed by a replica of \citet{Wang+Mokhtarian:2024} illustrating nuances of the interface (Section~\ref{sec:wang-replica}). Finally, we show that \pkg{OPSR} can also estimate the classic Tobit-5 model and matches the results obtained with the implementation from \pkg{sampleSelection} (Section~\ref{sec:tobit-5}).

\subsection[OPSR core]{\pkg{OPSR} core} \label{sec:opsr-core}
...
\item \code{type = "unlog-response"}: Predicts the back-transformed response according to Equations~\ref{eq:log-cond-exp}-\ref{eq:log-counterfact-exp} if the continuous outcome was log-transformed (either in the \code{formula} or during data pre-processing). The smoothing constant used during the continuity correction (i.e., the $\delta$ in $y_j = \ln(Y_j + \delta)$) can be specified via the \code{delta} argument and defaults to 1.
...
\item \code{type = "correction"}: Returns $\rho_j \sigma_j$IMR respectively $\rho_{j'} \sigma_{j'}$IMR (if \code{counterfact = j'} was specified) from Equation~\ref{eq:cond-exp} or \ref{eq:counterfact-exp}.
\item \code{type = "Xb"}: Returns $\Xb$ respectively $\Xbd$ (if \code{counterfact = j'} was specified) from Equation~\ref{eq:cond-exp} or \ref{eq:counterfact-exp}.
\end{itemize}
...
The function \fct{opsr\_te} wraps the required \fct{predict} calls and prepares the inputs for treatment effect computations returning an object of class \class{opsr.te}. A subsequent call to \fct{summary} actually computes the treatment effects (TE) and average treatment effects (ATE). An associated \fct{print} method presents the final computations. \fct{print.opsr.te} internally calls \fct{summary} and therefore the explicit call to \fct{summary} can be omitted, if the analyst does not require the underlying computed objects
%
<<opsr-te>>=
...
Last, there is a \fct{plot} method for model fits of class \class{opsr}. The method internally calls \fct{opsr\_te} and then \fct{pairs.opsr.te} which visualizes the treatment effects in a matrix of scatterplots. The plot method is demonstrated later in Section~\ref{sec:case-study}, Figure~\ref{fig:pairs}.

\subsection[Convergence diagnostics and identification tests]{Convergence diagnostics and identification tests} \label{sec:diagnostics}

The second-order optimality conditions at the MLE estimate can be inspected with \fct{opsr\_diagnostics}:
%
<<diagnostics>>=
opsr_diagnostics(fit)
@
%
The output reports (i) the maximum absolute gradient element (\code{max\_abs\_gradient}); (ii) whether the Hessian is negative definite (\code{hessian\_neg\_def}), a necessary condition for a proper local maximum rather than a saddle point; (iii) all eigenvalues; and (iv) the condition number (ratio of largest to smallest absolute eigenvalue). A large condition number --- typically above $10^3$--$10^4$ --- signals near-collinearity in the parameter space and suggests that some parameter combination is weakly identified.

These second-order conditions are \emph{necessary but not sufficient} to confirm empirical identification. A model with a weak or absent exclusion restriction can produce a gradient close to zero and a negative-definite Hessian while $\rho_j$ is essentially unidentified: the log-likelihood surface is flat in the $\rho_j$ direction, not ill-shaped. The condition number provides a soft signal, but cannot alone distinguish identification failure from benign covariate collinearity.

Two dedicated diagnostics address this. \fct{opsr\_lr\_instrument} performs a likelihood-ratio test that drops specified instrument(s) from the selection equation; a small LR statistic signals a weak instrument. \fct{opsr\_profile\_rho} fixes each $\rho_j$ on a grid and re-maximises over all other parameters, producing a profile log-likelihood curve per regime: a flat profile means the data carry almost no information about $\rho_j$ beyond the normality assumption. Both diagnostics are combined in \fct{opsr\_test\_exclusion\_restriction}:
%
<<exclusion-test-sim>>=
test_sim <- opsr_test_exclusion_restriction(fit, instrument = "xs2",
  grid = seq(-0.99, 0.99, length.out = 11), printLevel = 0)
print(test_sim)
@
%
Because the simulated data uses \code{xs2} as a genuine excluded instrument, the LR test is strongly significant and the profile is peaked, confirming that $\rho_j$ is well identified by the data. A plot of the profile can be produced with \code{plot(test\_sim\$profile\_rho)}. The contrast with the weak-instrument and no-exclusion conditions is shown in Section~\ref{sec:simulation}; a real-data application is in Section~\ref{sec:cs-identification}.

\subsection[Reproducing Wang and Mokhtarian (2024)]{Reproducing \citet{Wang+Mokhtarian:2024}} \label{sec:wang-replica}
...
@
%
where all ATEs are negative. Only \code{G3} (the current UTWers) would increase weekly VMD when switching from NTWing to NUTWing (\Sexpr{round(ste$te[1, 3], 3)} miles). All treatment effects are significantly different from zero, except \code{G2 T1->T2}, e.g., the NUTWers switching from NTWing to NUTWing.

\subsection[Tobit-5 model and comparison to sampleSelection]{Tobit-5 model and comparison to \pkg{sampleSelection}} \label{sec:tobit-5}

As noted in Section~\ref{sec:intro}, the Tobit-5 model can be seen as a form of OPSR with only two selection outcomes and can be fitted with the \proglang{R}-package \pkg{sampleSelection}. In this section, we illustrate that \pkg{OPSR} can estimate Tobit-5 models (as all the other examples involve three regimes) and that the results match the ones obtained with \pkg{sampleSelection}. The example, using simulated data, is directly taken from the vignette \citet[Section~4.2]{Toomet+Henningsen:2020} \code{vignette("selection", package = "sampleSelection")}.
...
To benchmark estimator performance and illustrate how the identification diagnostics behave under known conditions, we replicate and extend the Monte Carlo study of \citet{Chiburis+Lokshin:2007} for the three-regime OPSR case. Pre-computed results are available as the package data object \code{simulation\_study}:
%
<<sim-load>>=
data("simulation_study", package = "OPSR")
mc   <- simulation_study$monte_carlo
diag_sim <- simulation_study$diagnostics
@

\subsection{Design}

The data-generating process (DGP) mirrors \citet{Chiburis+Lokshin:2007}: two standard-normal regressors $x_1, x_2$; bivariate-normal errors with correlation $\rho = 0.5$ (except run~2); an ordered-probit selection rule with thresholds $\kappa = (-1, 1)$ yielding three regimes; and outcome $y = \beta x_1 + \varepsilon$ (true $\beta = 1$). Variable $x_2$ enters the selection equation with coefficient $\alpha_2 = 1$ and is excluded from the outcome equation --- the exclusion restriction. Six conditions are run with $R = 100$ replications at $N = 1000$ (the baseline also varies $N \in \{50, 100, 200, 300, 500\}$):

\begin{enumerate}
\item \textbf{Baseline}: textbook DGP, valid exclusion restriction, six sample sizes.
\item \textbf{High $\rho = 0.99$}: severe selection bias.
\item \textbf{Non-normal shocks}: squared standard-normal errors (mean-centred).
\item \textbf{No exclusion restriction}: $x_2$ absent from the selection equation ($\alpha_2 = 0$) and the outcome equation; identification via functional form only.
\item \textbf{Violated exclusion restriction}: $x_2$ in the selection equation ($\alpha_2 = 1$) but also enters the outcome directly ($\beta_{x_2} = 1$); instrument is invalid.
\item \textbf{Weak instrument}: $x_2$ in the selection equation with $\alpha_2 = 0$ (no predictive power) but correctly excluded from outcomes.
\end{enumerate}

Results are stored for both the FIML estimator (default \fct{opsr}) and the Heckman two-step (\fct{opsr\_2step}); run~3 is omitted from the paper (included in the stored object) to match the numbering of \citet{Chiburis+Lokshin:2007}.

\subsection{Parameter recovery}

Table~\ref{tab:sim} reports FIML mean $\hat\beta$ and 95\% CI coverage across the three regimes at $N = 1000$.
%
<<sim-coverage, results=tex, echo=FALSE>>=
fiml <- mc[mc$estimator == "FIML" & mc$n_obs == 1000, ]

run_labels <- c(
  "1_baseline"      = "Baseline",
  "2_high_rho"      = "High $\\rho = 0.99$",
  "4_nonnormal"     = "Non-normal shocks",
  "5_no_excl"       = "No excl.~restriction",
  "6_excl_violated" = "Excl.~restriction violated",
  "7_weak_inst"     = "Weak instrument"
)

make_row <- function(run_id) {
  sub <- fiml[fiml$run == run_id, ]
  sub <- sub[order(sub$regime), ]
  vals <- vapply(seq_len(nrow(sub)), function(i) {
    sprintf("%.3f (%.0f\\%%)", sub$mean_beta[i], sub$coverage[i])
  }, character(1))
  c(run_labels[run_id], vals)
}

tab <- do.call(rbind, lapply(levels(fiml$run), make_row))

cat("\\begin{table}[htbp]\n\\centering\n")
cat("\\caption{\\label{tab:sim} FIML mean $\\hat\\beta$ and 95\\% CI coverage at ",
    "$N = 1000$ across three regimes. True $\\beta = 1$.}\n")
cat("\\begin{tabular}{lccc}\\toprule\n")
cat("Condition & Regime~1 & Regime~2 & Regime~3 \\\\\n\\midrule\n")
...
@
%
The baseline and high-$\rho$ conditions achieve near-nominal coverage for all three regimes with negligible bias, confirming that FIML performs well when a valid exclusion restriction is in place. Non-normal shocks (run~3 in the original paper) primarily hurt the exterior regimes (1 and 3), while the interior regime~2 remains close to nominal because the near-linearity of its Mills-ratio correction makes FIML approximately equivalent to a linear correction there.

The identification conditions reveal sharply different failure modes. With no exclusion restriction or a weak instrument (runs~5 and~7), coverage collapses substantially for the interior regime~2 --- exactly the regime predicted by \citet{Chiburis+Lokshin:2007} to suffer most from functional-form-only identification. With a violated exclusion restriction (run~6), all estimates are tightly concentrated but far from the truth (coverage $\approx 0\%$ for all regimes): the model converges confidently to the wrong answer.

\subsection{Diagnostic performance}

Table~\ref{tab:sim-diag} summarises the output of \fct{opsr\_test\_exclusion\_restriction} for one representative dataset per condition.
%
<<sim-diag, results=tex, echo=FALSE>>=
diag_r2 <- diag_sim[diag_sim$regime == 2, ]
diag_r2 <- diag_r2[order(diag_r2$run), ]
...
cat("\\begin{table}[htbp]\n\\centering\n")
cat("\\caption{\\label{tab:sim-diag} Diagnostics for one representative dataset ",
    "per condition ($N = 1000$). Profile range is the log-likelihood drop ",
    "across the $\\rho$ grid for the interior regime~2; smaller values ",
    "indicate a flatter (weaker) profile.}\n")
cat("\\begin{tabular}{lccc}\\toprule\n")
cat("Condition & Cond.~number & Profile range & LR $p$-value \\\\\n\\midrule\n")
for (i in seq_len(nrow(diag_r2))) {
  run_id <- as.character(diag_r2$run[i])
  label  <- run_labels2[run_id]
  pval   <- fmt_p(diag_r2$lr_pval[i])
  cat(sprintf("%s & %.0f & %.1f & %s \\\\\n",
              label, diag_r2$cond_number[i],
              diag_r2$profile_range[i], pval))
}
cat("\\bottomrule\n\\end{tabular}\n\\end{table}\n")
@
%
The diagnostics correctly signal identification problems for runs~5 and~7: the condition number is an order of magnitude larger than the baseline ($> 900$ vs.\ $\sim 34$), the profile log-likelihood for regime~2 is near-flat (range $< 2$ LL units, compared to $> 90$ for the baseline), and the LR test is non-significant. For a valid but strong instrument (run~1) or high error correlation (run~2) these quantities show healthy values.

Critically, the violated exclusion restriction (run~6) is \emph{not} flagged: the LR test is strongly significant, the profile is curved, and the condition number is similar to the baseline. The model estimates $\rho_j$ with apparent precision, but the estimates are wrong because the normality-driven correction has absorbed the misspecified direct effect of $x_2$ on the outcome. This is the key practical limitation: instrument validity requires subject-matter justification, not just a significant LR test. Practitioners should therefore apply \fct{opsr\_test\_exclusion\_restriction} as a \emph{necessary} check, while treating a clean result as evidence of instrument \emph{relevance}, not validity.
...
plot.it()
@
\caption{\label{fig:boxplot} Log weekly distance traveled and log one-way commute distance for different telework statuses.}
\end{figure}

%% The model
As discussed in Section~\ref{sec:identification}, the analyst must designate valid exclusion restrictions: variables that enter the selection equation but are excluded from all outcome equations. In our application, we use occupation type (ISCO-08 categories) and shift-work status as instruments --- these variables predict telework adoption (professional and managerial workers telework more; shift workers cannot) but are not expected to directly affect weekly travel distance after controlling for sociodemographic and residential variables. To simplify model specification, we first estimate the ordered probit model separately, using \fct{polr} from the \pkg{MASS} package \citep{Venables+Ripley:2002}. It should be noted here, that the resulting parameter estimates of the selection process are unbiased.
%
<<fit-polr>>=
...
@
%
A caveat is warranted here and for the significance-based outcome pruning that follows: automated variable selection can inadvertently create invalid exclusion restrictions. If a covariate is retained in the selection equation but pruned from all outcome equations, it silently becomes an instrument --- one whose validity must be verified by subject-matter reasoning. We validate the exclusion restriction explicitly in Section~\ref{sec:cs-identification}. Additionally, reported p-values for selected variables are optimistic due to post-selection bias.

The resulting selection specification guides the choice of instruments: occupation type (ISCO-08) and shift-work status enter the selection equation as exclusion restrictions. Workers in craft, technical, and clerical occupations face different telework feasibility constraints that plausibly affect \emph{telework adoption} but have no direct effect on weekly travel distance after controlling for sociodemographic variables. The resulting selection process specification can then be passed to \fct{opsr}, along with separate process specifications for the outcome equations.

We then follow a conventional (somewhat heuristic) model building strategy: specify the full identified model and then exclude all variables that do not produce significant estimates (at the 10\% level). The function \fct{opsr\_step} can help in this iterative process, as it excludes all coefficients from the model specification with $p$~values below some threshold (see \code{?opsr_step} for further details). The formula specification of the full model is hidden here for brevity; note that \code{car\_access} and \code{parking\_home} appear in the outcome equations (not in the selection equation) because vehicle accessibility directly predicts travel distance but is not expected to primarily determine telework status after conditioning on occupation type.
%
<<hidden, echo=FALSE>>=
f_full <- wfh | log_weekly_km ~
  ...
  isco_elementary + parking_home + freq_onl_order +
  grocery_shopper |
...
@
%
The reduced model specification (\code{fit_red}) is not rejected in the likelihood ratio test. Further, the Wald-test of joint significance of the \code{rho} coefficients can be assessed from the summary output; rejection of the null (\code{rho1} = \code{rho2} = \code{rho3} = 0) indicates that the selection correction is beneficial given our model assumptions.

%% Exclusion restriction validation
\subsection{Exclusion restriction and identification} \label{sec:cs-identification}

Before interpreting treatment effects, we validate the exclusion restriction of the fitted model. The instruments are the occupation and shift-work variables that appear in the selection equation but not in the outcome equations. We test their joint relevance with \fct{opsr\_test\_exclusion\_restriction} and inspect the per-regime profile likelihood for $\rho_j$:
%
<<exclusion-test-cs>>=
instruments_cs <- c("shift_work", "isco_craft", "isco_tech", "isco_clerical")
test_cs <- opsr_test_exclusion_restriction(fit_red,
  instrument = instruments_cs,
  grid = seq(-0.99, 0.99, length.out = 7), printLevel = 0)
print(test_cs)
@
%
The LR test is strongly significant (the instruments jointly improve the selection fit well beyond chance), confirming instrument relevance. The profile log-likelihood plots per regime are produced by
%
\setkeys{Gin}{width=\textwidth}
\begin{figure}[t!]
\centering
<<profile-cs, echo=TRUE, fig=TRUE, height=3, width=9>>=
plot(test_cs$profile_rho)
@
\caption{\label{fig:profile} Profile log-likelihood for $\rho_j$ per regime. Each panel fixes $\rho_j$ at the grid value and re-maximises over all other parameters. Peaked profiles indicate that $\rho_j$ is identified by the data.}
\end{figure}
%
Peaked profiles for all three regimes confirm that the error correlations are empirically identified, not merely pinned by the bivariate normality assumption. This stands in contrast to the flat profiles produced when no exclusion restriction is present (Section~\ref{sec:simulation}). The condition number from \fct{opsr\_diagnostics} provides a secondary confirmation:
%
<<diagnostics-cs>>=
opsr_diagnostics(fit_red)
@
%
A negative-definite Hessian and a moderate condition number indicate a well-behaved likelihood surface at the MLE.

%% The treatment effects
\subsection{Treatment effects} \label{sec:cs-te}

We first define a helper function (wrapping \fct{opsr\_te}), that provides more intuitive labels for the treatment effects, simplifying the discussion that follows. Unless otherwise mentioned, we use the \code{fit_red} model in the remainder.
%
<<utils>>=
te <- function(fit) {
...
@
\caption{\label{fig:pairs} Pairs plot for model fits of class \class{opsr}.}
\end{figure}

\begin{table}[htbp]
\centering
\begin{tabular}{|l|l|>{\centering\arraybackslash}p{2cm}|p{7.4cm}|}
\hline
Model & Parent & \centering Error \\ correlation & Description \\
\hline
...
\hline
\end{tabular}
\caption{\label{tab:model-overview} Model overview. The model is based on \emph{Parent} as elaborated under \emph{Description}.}
\end{table}
...
The package provides two layers of model-quality diagnostics. \fct{opsr\_diagnostics} inspects the second-order optimality conditions at the MLE (gradient magnitude, Hessian negative-definiteness, condition number), addressing whether the optimizer found a well-defined maximum. \fct{opsr\_test\_exclusion\_restriction} goes further by testing instrument relevance via a likelihood-ratio test and examining whether $\rho_j$ is empirically identified beyond distributional assumptions via the profile log-likelihood. Together these diagnostics make identification failure modes~---~absent instruments, weak instruments~---~visible before treatment effects are computed. The key limitation, shared with all Heckman-type models, is that an \emph{invalid} exclusion restriction (instrument that belongs in the outcome equation) cannot be detected from within the model and requires substantive judgment.

A Monte Carlo study replicating \citet{Chiburis+Lokshin:2007} for the three-regime case confirms: FIML achieves near-nominal coverage when a valid instrument is in place; coverage collapses (especially for interior regimes) when identification relies on functional form alone; and a violated exclusion restriction produces confident but catastrophically wrong estimates with 0\% coverage. The simulation results are stored as the package data object \code{simulation\_study} and can be used as a benchmark for future extensions.

In the case study, the OPSR method is applied to a tracking and activity diary dataset collected in Switzerland, investigating the telework treatment effects on weekly distance traveled across all modes. We demonstrate, first, how to specify an appropriately identified model (using occupation-type and shift-work variables as exclusion restrictions), how to validate the exclusion restriction using the diagnostic tools, and second, in how far computed treatment effects differ if the error correlation is not accounted for. We find that, overall, telework reduces travel. Non-teleworkers tend to have shorter commutes and adjust mobility patterns mainly when switching from non-usual telework to usual telework. On the other hand, weekly distance traveled slightly increases when initially adopting some telework. Contrary, usual teleworkers (had they not been teleworking) adjust mobility patterns strongly when adopting some telework but then only marginally adjust distance traveled when further adopting telework. Comparing the unit treatment effects to the two-way commute distance indicates that telework generally reduces weekly distance traveled and it does so by less than the foregone commute. Therefore, some compensating travel (rebound effects) exists for most of the teleworker groups.
...
\pkg{OPSR}~\Sexpr{packageVersion("OPSR")}, \pkg{MASS}~\Sexpr{packageVersion("MASS")}, \pkg{texreg}~\Sexpr{packageVersion("texreg")}, \pkg{sampleSelection}~\Sexpr{packageVersion("sampleSelection")}, \pkg{mvtnorm}~\Sexpr{packageVersion("mvtnorm")}, \pkg{gridExtra}~\Sexpr{packageVersion("gridExtra")} and \pkg{gridGraphics}~\Sexpr{packageVersion("gridGraphics")}. \proglang{R} itself
and all packages used are available from the Comprehensive
\proglang{R} Archive Network (CRAN) at
\url{https://CRAN.R-project.org/}.

Please note that slight differences in the results might occur depending on the platform. The results presented here were generated on \Sexpr{gsub("_", "\\\\_", R.version$platform, fixed = TRUE)}.
