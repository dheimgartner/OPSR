...
\item \code{type = "Xb"}: Returns $\Xb$ respectively $\Xbd$ from Equation~\ref{eq:cond-exp} or \ref{eq:counterfact-exp} (depending on whether or not the \code{counterfact} argument is specified).
\end{itemize}
...
@
%
The resulting selection process specification can then be passed to \fct{opsr}, along with a common (or separate) process specification for the outcome processes. \pkg{OPSR} recognizes potential identification problems (e.g., colinear variables or missing factor levels in one of the groups), raises a warning if such problems arise and fixes the causing coefficients at 0. Here, we have two singularity issues for the UTWers: First, \code{shift_work} is a constant and second, \code{parking_home} is colinear with \code{car_access}.
...
\item \fct{te}: Computes the treatment effect from an \code{"opsr"} fit object by combining all the steps above.
\end{itemize}
...
@
%
While resulting treatment effects based on \code{fit_red} are comparable to the ones based on \code{fit_commute} (in terms of both the direction and magnitude), \code{fit_nocor} yields completely different insights, in particular, that telework generally increases weekly distance traveled.
...
Xinyi Wang and Daniel Heimgartner conceived of the presented idea to formalize the learnings by \citet{Wang+Mokhtarian:2024} in an \proglang{R} package. The theory presented in Section~\ref{sec:model} stems from that work. Daniel Heimgartner implemented the functionality and \proglang{R} package architecture based on Xinyi Wang's original scripts underlying \citet{Wang+Mokhtarian:2024}, as well as drafted the paper. All authors discussed the results and contributed to the final manuscript.
