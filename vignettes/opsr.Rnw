...
@
%
which does not reject the null that the simpler model is sufficient. It should be noted that the modeled covariance matrix (in particular the magnitude of \code{rho}) potentially strongly influences the treatment effects.
