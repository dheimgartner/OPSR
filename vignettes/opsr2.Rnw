...
Last, there is a \fct{plot} method for model fits of class \class{opsr}. The method wraps \fct{pairs.opsr.te} and visualizes the treatment effects in a matrix of scatterplots as demonstrated in Section~\ref{sec:case-study}.
...
We first define a helper function, that provides more intuitive labels for the treatment effects, simplifying the discussion that follows. Unless otherwise mentioned, we use the \code{fit_red} model in the remainder.
%
<<utils>>=
te <- function(fit) {
...
plot.it()
@
\caption{\label{fig:treatment-effects} Treatment effects.}
\end{figure}
